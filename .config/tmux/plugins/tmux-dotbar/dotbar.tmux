...
session_attr=""
[ "$bold_session" = true ] && session_attr=",bold"
...
if [ "$rounded" = "true" ]; then
  ...
  session_color="#{?client_prefix,$fg_prefix,$fg_session}"
  session_fill_attr=""
  [ "$bold_session" = true ] && session_fill_attr=",bold"
  session_component="#[bg=$bg,fg=$session_color]$edge_left#[bg=$session_color,fg=$bg$session_fill_attr]$session_text_inner#[bg=$bg,fg=$session_color]$edge_right "
else
  session_component="#[bg=$bg,fg=$fg_session$session_attr]#{?client_prefix,,$session_text}#[bg=$fg_prefix,fg=$bg,$bold_attr]#{?client_prefix,$session_text,}#[bg=$bg,fg=${fg_session}]"
fi
